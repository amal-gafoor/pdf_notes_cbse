...
// The embeddable twin of driveUrl. Drive serves /view with
// X-Frame-Options: SAMEORIGIN but leaves /preview frameable — that endpoint
// exists precisely for this. The bytes still come from Google straight to the
// student's browser, so an in-app preview costs no origin transfer either.
//
// Like driveUrl, this only renders while the folder is link-shared.
export function previewUrl(file) {
  const editors = {
    [`${GOOGLE_NATIVE}.document`]: "document",
    [`${GOOGLE_NATIVE}.spreadsheet`]: "spreadsheets",
    [`${GOOGLE_NATIVE}.presentation`]: "presentation",
  };
  const kind = editors[file.mimeType];
  // Docs/Sheets/Slides live under docs.google.com and never at /file/d/.
  if (kind) return `https://docs.google.com/${kind}/d/${file.id}/preview`;
